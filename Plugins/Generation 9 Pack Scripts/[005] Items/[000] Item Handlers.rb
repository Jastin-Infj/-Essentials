...
ItemHandlers::UseOnPokemon.add(:SUPERCAPSULE, proc { |item, qty, pkmn, scene|
  if scene.pbConfirm(_INTL("Do you want to change {1}'s ability?", pkmn.name))
    ...
    if abils.length < 2 && !pkmn.hasHiddenAbility? || pkmn.isSpecies?(:ZYGARDE)
            scene.pbDisplay(_INTL("It will not effect {1}.",pkmn.name))
        else
    ability_commands = []
    abil_cmd = 0
    for i in abils
    ...
    cmd= pbMessage("Choose a new ability for your Pokémon.",ability_commands,cmdIfCancel=-1,nil,0)
    next false if cmd == -1
    pkmn.ability = abils[cmd][0]
    scene.pbDisplay(_INTL("You've altered {1}'s ability!", pkmn.name))
    abils.each do |i|
    end
  end
end
